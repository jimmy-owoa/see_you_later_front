  ...
  <div>
    ...
    <v-card class="mx-auto" max-width="400">
      <v-list-item two-line>
        <v-list-item-content>
          <v-list-item-title class="headline">{{data.event.title}}</v-list-item-title>
          <v-list-item-subtitle>{{data.event.date}}</v-list-item-subtitle>
        </v-list-item-content>
      </v-list-item>
  ...
  async asyncData({isDev, route, store, env, params, query, req, res, redirect, error}){
    try {
      const res = await axios.get(
        `http://localhost:3000/invitations/${params.id}`
      );
      const data = res.data;
      return { data };
    } catch (error) {
